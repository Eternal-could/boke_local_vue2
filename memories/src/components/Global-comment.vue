<template>
  <div>
    <div v-for="(commentItem, index) of localComment"
      :key="index"
      class="comment-card"
    ...
    >
      <el-row :gutter="10">
        <el-col :span="2">
          <el-avatar :src="commentItem.userData.avatar" size="medium"></el-avatar>
        </el-col>
        <el-col :span="6">
          <span>{{commentItem.userData.userName}}</span>
          <br>
          <span style="display: inline-block;margin-top: 6px">{{commentItem.lastModified | transformTime}}</span>
        </el-col>
      </el-row>
      <el-row :gutter="10">
        ...
        <el-col :span="24" style="border-left: 2px solid #cccccc; margin-top: 20px">
          <div v-for="(sonCommentItem, index) of commentItem.comment"
               :key="index"
               style="margin: 20px; border-bottom: 1px dashed #c0c4cc"
          >
            <el-row :gutter="10">
              <el-col :span="2">
                <el-avatar :src="sonCommentItem.userData.avatar" size="medium"></el-avatar>
              </el-col>
              <el-col :span="6" style="font-size: 14px;margin-bottom: 12px">
                <span>{{sonCommentItem.userData.userName}}</span>
                <br>
                <span style="display: inline-block;margin-top: 6px">{{sonCommentItem.lastModified | transformTime}}</span>
              </el-col>
              <el-col :span="16" style="text-indent: 2em; color: #483d8b">
      ...
      <el-row :gutter="10" v-if="commentItem.isComment">
        <el-col :span="24">
          <el-input v-model="commentStr" placeholder="请输入回复的内容"></el-input>
        </el-col>
        <el-col :span="24" style="text-align: right; margin-top: 20px">
          <el-button type="danger" @click="cancelComment(commentItem,'son')">取消</el-button>
    ...
    <el-row :gutter="10" v-if="isComment">
      <el-col :span="24">
        <el-input v-model="articleCommentStr" placeholder="请输入回复的内容"></el-input>
      </el-col>
      <el-col :span="24" style="text-align: right; margin-top: 20px">
        <el-button type="danger" @click="cancelComment(localComment,'father')">取消</el-button>
...
export default {
  ...
  methods: {
    switchComment(commentItem) {
      ...
      //   this.isComment = false;  // 如果我们对某个回复框做评论，那么就关掉它
      // } else {
      //   this.isComment = true;  // 如果我们对某个回复框做评论，那么就打开它
      // }
      this.isComment = !commentItem.isComment;
    },
